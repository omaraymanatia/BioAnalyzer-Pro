...
function App() {
  ...
  const validateSequence = (seq: string): boolean => {
    ...
    const badChars = advancedAnalysis.findBadCharacters(seq);
    if (badChars.length > 0) {
      setResult(`Invalid characters found in sequence: ${badChars.join(', ')}`);
      return false;
    }
    return true;
  };
  ...
  const handleAnalysis = (type: string) => {
    try {
      ...
      switch (type) {
        ...
        case 'bad_chars':
          const badChars = advancedAnalysis.findBadCharacters(dnaSequence);
          setResult(
            badChars.length > 0
