...
export const advancedAnalysis = {
  ...
  findBadCharacters: (sequence: string): string[] => {
    return [...new Set(sequence.toUpperCase().split(''))]
      .filter(char => !VALID_DNA_BASES.has(char));
  },
  ...
  queryPattern: (text: string, pattern: string): number => {
    const indices = advancedAnalysis.indexSorted(text);
